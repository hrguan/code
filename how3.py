...
3堆（Heap or Priority Queue）、栈（Stack）、队列（Queue）、哈希表类（Hashmap、Hashset）：
...
7深度优先搜索（DFS）：面试中最常考的（分类的稍微有点粗糙了，没有细分出回溯/分治来，准备找个时间给每个DFS的题标记下是哪种DFS）
...
9并查集（Union Find）：把两个或者多个集合合并为一个集合
...
11单调栈与单调队列（Monotone Stack／Queue）
